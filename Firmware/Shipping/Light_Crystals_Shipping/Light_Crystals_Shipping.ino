...
//
// FastLED library MUST be the latest MASTER branch downloaded from their GitHub repo
...
float noiseFloor = 4.0f;  // Start with a reasonable initial guess for the noise floor.
const float minNoiseFloor = 1.7f;  // Minimum level to prevent the noise floor from dropping too low.
const float noiseFloorDecay = 0.85f;  // Controls how quickly the noise floor decays.
const float noiseFloorRise = 0.05f;   // Controls how quickly the noise floor adapts upward.
float raw_divider = 8388608.0f;  // Initial scaling factor to normalize 32-bit input.
float min_raw = 0;
float max_raw = 0;
float _raw = 0;
int maxBandValue = 0;
...
bool do_fft_calcs()
{
  int samples_read = i2s.readBytes((char *)raw_samples, sizeof(int32_t) * BLOCK_SIZE);
  ...
  for (uint16_t i = 0; i < BLOCK_SIZE; i++) {
    vReal[i] = (float)raw_samples[i] / (raw_divider / 20); // Normalize the input.
    //    vReal[i] = (float)(raw_samples[i] << 8);
    vImag[i] = 0.0f;  // Set imaginary part to zero.

    if (raw_samples[i] > max_raw) max_raw = (float)raw_samples[i];
    if (raw_samples[i] < min_raw) min_raw = (float)raw_samples[i];
  }
  ...
  {
    raw_divider = max(raw_divider - 100.0f, 1000.0f);  // Ensure raw_divider doesn't go too low.
  }
  ...
  for (int i = 2; i < (BLOCK_SIZE / 2); i++) {
    ...
    if (i <= 2) bands[0] += (int)magnitude;
    else if (i <= 4) bands[1] += (int)magnitude;
    else if (i <= 6) bands[2] += (int)magnitude;
    else if (i <= 14) bands[3] += (int)magnitude;
    else if (i <= 28) bands[4] += (int)magnitude;
    else if (i <= 50) bands[5] += (int)magnitude;
    else if (i <= 120) bands[6] += (int)magnitude;
    else if (i <= 190) bands[7] += (int)magnitude;
    else bands[8] += (int)magnitude;
  ...
  if (totalVolume > noiseFloor) {
    ...
    noiseFloor = min((1.0f - noiseFloorRise) * noiseFloor + noiseFloorRise * totalVolume, 10.0f);
  } else {
  ...
  //  Serial.printf("SET noiseFloor: %f  maxBandValue: %d, max_band_val: %d\n\n", noiseFloor, maxBandValue, max_band_val);
